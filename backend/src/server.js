...
app.post('/api/ai/dailyQuote', async (req, res) => {
  ...
  if (isTimerDescription) {
    ...
    sys = `${minChars}~${maxChars}자 한국어 완전한 문장 하나만 출력. 여러 문장 절대 금지. 한 문장만. 존대어 사용(반말 금지). 활기 넘치고 응원하는 경향으로 작성. 쉬면서 일하는 것의 효과를 강조: "쉬면서 일하면 효율이 오릅니다.", "오늘도 쉬엄쉬엄 일하세요.", "적절한 휴식이 생산성을 높입니다." 등의 메시지를 포함. 휴식이 건강과 생산성에 도움이 된다는 것을 명확히 전달. 맨 마지막에 문장 내용에 맞는 이모지를 반드시 포함하되, 사람이 나오는 이모지는 사용하지 말고 하트(❤️, 💚, 💙 등), 웃음(😊는 제외), 🌿, ☕, 🍵 중점적으로 사용해야 함. 한국어 맞춤법이 완벽해야 함.`;
  } else {
