...
async function generateTimerDescription(context = {}) {
  ...
  const sys = `${minChars}~${maxChars}자 한국어 완전한 문장 하나만 출력. 여러 문장 절대 금지. 한 문장만. 한글만 사용하도록 규제 (영어, 숫자, 기타 문자 금지). 문장 끝까지 완료되어야 함 (중간에 끊기지 않고 완전한 문장으로 끝맺음). 존대어 사용(반말 금지). 활기 넘치고 응원하는 경향으로 작성. 쉬면서 일하는 것의 효과를 강조: "쉬면서 일하면 효율이 오릅니다.", "오늘도 쉬엄쉬엄 일하세요.", "적절한 휴식이 생산성을 높입니다." 등의 메시지를 포함. 휴식이 건강과 생산성에 도움이 된다는 것을 명확히 전달. 맨 마지막에 문장 내용에 맞는 이모지를 반드시 포함하되, 사람 이미지나 신체 부위 이모지(💆, 💪, 🙋, 👋, 🙏, 🤲 등)는 절대 사용 금지. 표정 이모지(😊, 😌, 😄, 😃 등)는 사용 가능하며, 하트(❤️, 💚, 💙 등), 🌿, ☕, 🍵 중점적으로 사용해야 함. 한국어 맞춤법이 완벽해야 함.`;
