...
export async function requestDailyAffirmation(context = {}, { timeoutMs = 1500 } = {}) {
  ...
  try {
    const res = await fetch(`${getApiBase()}/api/ai/dailyQuote`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ context, constraints: { exactChars: 12, tone: 'warm', witty: true, suffixEmoji: true, seedPhrase: '쉬면서 일해야 능률이 올라가요!' } }),
      signal: ctrl.signal,
    });
    if (!res.ok) throw new Error(`AI HTTP ${res.status}`);
    const data = await res.json();
...
export async function requestDailyAffirmationBatch(context = {}, count = 20, { timeoutMs = 2500 } = {}) {
  ...
  try {
    const res = await fetch(`${getApiBase()}/api/ai/dailyQuoteBatch`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ context, count, constraints: { exactChars: 12, tone: 'warm', witty: true, suffixEmoji: true, seedPhrase: '쉬면서 일해야 능률이 올라가요!' } }),
      signal: ctrl.signal,
    });
    if (!res.ok) throw new Error(`AI HTTP ${res.status}`);
    const data = await res.json();
