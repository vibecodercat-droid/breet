...
export async function recommendNextBreakWithAI(expectedBreakMinutes, excludeIds = []) {
  ...
  const instructions = [
    '당신은 브레이크 코치입니다.',
    `아래 제약에 맞춰 추천 후보 정확히 3개를 JSON으로만 반환하세요: {"suggestions":[{id,type,duration,description,howTo}]}.`,
    '- duration은 allowedDurations 중 하나만 사용(분 단위).',
    `- description은 한글로만 6-20자, 활동 설명 문장으로 작성.`,
    `- howTo는 수행 방법을 8-16자 한국어로 간결히 작성(예: '먼 곳 20초 보기').`,
  ...
  try {
    ...
    const normalized = list.slice(0, 3).map((s) => {
      ...
      const howTo = isValidHowTo(s?.howTo) ? s.howTo.trim() : null;
      return { id: base.id, type: base.type, duration: allowedDuration, name: `${allowedDuration}분 ${desc}`, hint: howTo || '' };
    }).filter(Boolean);
    ...
    await chrome.storage.local.set({ pendingBreakCandidates: filled, pendingBreak: top, aiSource: 'ai' });
    ...
    await chrome.storage.local.set({ pendingBreakCandidates: fallbackSet, pendingBreak: top, aiSource: 'rule' });
