...
export async function recommendNextBreakWithAI(expectedBreakMinutes, excludeIds = []) {
  ...
  const instructions = [
    '당신은 브레이크 코치입니다.',
    `아래 제약에 맞춰 추천 후보 정확히 3개를 JSON으로만 반환하세요: {"suggestions":[{id,type,duration,description}]}.`,
    '- duration은 allowedDurations 중 하나만 사용(분 단위).',
    `- description은 최종 표기명(제목)으로 사용되며 한국어 맞춤법을 준수. 8~20자. 불필요한 마침표 금지.`,
    `- 다음 ID는 제외하세요: ${excludeIds.join(', ') || '없음'}.`,
    '- 최근 동일 타입 반복은 피하고, 사용자의 preferredBreakTypes를 우선.',
    `- 작업/건강 관심사, 오늘/어제 투두, 선택된 휴식시간(${allowedDuration}분)에 맞춰 다양하게 제안. 서로 다른 type을 우선.`,
  ].join(' ');
